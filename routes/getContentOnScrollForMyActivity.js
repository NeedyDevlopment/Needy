...
router.post("/getPosts", async(req, res, next) => {
    ...
    const usersPost = await Post.find({ "creator._id": currentUserId._id }).populate("creator").skip(5 * (currentPage - 1)).limit(5).sort(
        "-date"
    );
...
router.post("/getActivities", async(req, res, next) => {
    ...
    const userActivities = await Activity.find({ userId: currentUserId._id }).skip(12 * (currentPage - 1))
        .limit(12).
    populate("post")
        .populate("creator")
        .sort("-date");
