...
router.post("/", (req, res, next) => {
  const currentUser = req.session.token ? jwt.verify(req.session.token, process.env.jwtPrivateKey) : null;
  const emailTo = req.body.emailTo;
  const postId = req.body.postId;
  // let verifier = new Verifier("jainishkunjadiya.freelancer@gmail.com", "ahpatel9@");
  ...
  var transporter = nodemailer.createTransport({
    service: "gmail",
    auth: {
      user: "jainishkunjadiya.freelancer@gmail.com",
      pass: "Ja@123456789",
    },
  });

  var mailOptions = {
    from: "jainishkunjadiya.freelancer@gmail.com",
    to: emailTo,
  ...
  transporter.sendMail(mailOptions, async function (error, info) {
    if (error) {
      ...
      console.log("info::::", info);
      const currentUserId = await _.pick(jwt.verify(req.session.token, process.env.jwtPrivateKey), ["_id"]);
      ...
      return res.status(200).json({ message: "Post shared successfully to " + emailTo });
    }
  });
});
