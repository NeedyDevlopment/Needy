...
app.get("/myactivity", AuthForLogin, async(req, res, next) => {
    ...
    const userActivities = await Activity.find({ userId: currentUserId._id }).populate('post');
    console.log("user Activities:::::;");
    console.log(userActivities);
    // const params = { likes: 10, comments: 20 };
    res.status(200).render("myActivity.pug", { posts: usersPost, userActivities: userActivities, isLoggedIn: req.session.isLoggedIn });
    next();
});
...
app.post("/editPost/:action", (req, res, next) => {
    ...
    if (req.params.action == 'Edit') {
        ...
        Post.findOneAndDelete({ _id: postId }).then(result => {
...
app.post("/sendMail", (req, res, next) => {
        const currentUser = req.session.token ? jwt.verify(req.session.token, "MySecureKey") : null;
