...
emitter.on('postAdded', async(args) => {
    const followersArrayDoc = await User.findById(args.creatorId).select(['followersArray', 'username']);
    console.log('FollowersArrayDoc is::::::::::::::::::::: ');
    console.log(followersArrayDoc);
    const followersArray = followersArrayDoc.followersArray.map(stringId => mongoose.Types.ObjectId(stringId));
    ...
    console.log(followersDoc);
    const followers = followersDoc.map(follower => { return { email: follower.email, username: follower.username }; });
...
app.get("/", AuthForRegister, async(req, res, next) => {
    ...
    const currentUserFollowingsArray = await User.findOne({ _id: currentUserId._id }).select('followingsArray');
    ...
    res.status(200).render("homepage.pug", { posts: postsArray, currentUserId: currentUserId._id, currentUserFollowingsArray: currentUserFollowingsArray.followingsArray, isLoggedIn: req.session.isLoggedIn, message: message });
...
app.post('/createpostSubmission', upload.single('image'), async(req, res, next) => {
    ...
    var creator = _.pick(jwt.verify(req.session.token, 'MySecureKey'), ['_id', 'username', 'email']);
    const creatorsFollowers = await User.findOne({ _id: currentUserId._id }).select('followers');
    ...
    const params = { data: post.image.data, contentType: post.image.contentType };
...
app.post('/ajax/:action', async(req, res, next) => {
    ...
    if (req.params.action === 'like') {
        ...
        if (incLikes == 1) {
            ...
            const activity = new Activity({ userId: currentUserId, activity: 'liked', date: Date.now(), postId: postId });
            ...
            const activity = new Activity({ userId: currentUserId, activity: 'unliked', date: Date.now(), postId: postId });
            const activityResult = await activity.save();
            console.log('activity result:::::::');
            console.log(activityResult);
            console.log('result is:;;');
        ...
        const postId = req.body.postId;
        var result1 = await Post.findByIdAndUpdate(postId, {
            $addToSet: { 'savedArray': currentUserId._id }
        }, { new: true });
        var result2 = await User.findByIdAndUpdate(currentUserId._id, {
            $addToSet: { 'savedPosts': postId }
        }, { new: true });
        const activity = new Activity({ userId: currentUserId, activity: 'saved', date: Date.now(), postId: postId });
        const activityResult = await activity.save();
        console.log('activity result:::::::');
        console.log(activityResult);
        ...
        var commentedBy = await User.findOne({ _id: currentUserId._id }).select('username photo');
        var result = await Post.findByIdAndUpdate(postId, {
            $push: {
                'commentedArray': {
        ...
        const activity = new Activity({ userId: currentUserId, activity: 'commented', commentText: commentText, date: Date.now(), postId: postId });
        const activityResult = await activity.save();
        var result1 = await Post.findByIdAndUpdate(postId, {
